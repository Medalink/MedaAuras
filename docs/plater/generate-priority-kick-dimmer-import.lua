...
local options = {
    {
        Type = 2,
        Name = "Cast Dim Opacity",
        Desc = "Controls how visible non-casting enemy NPC plates remain while the dimmer is active. 0 makes other plates fully invisible, 25 is a heavy dim, and 100 disables the visual dim effect.",
    ...
    {
        Type = 4,
        Name = "Trigger On Focus Target Casts",
        Desc = "If your current focus target has a visible enemy NPC nameplate and starts casting or channeling, the dimmer will trigger even outside the normal priority list. This is checked before the general any-cast and interrupt modes.",
        Key = "matchFocusTargetCasts",
        Value = false,
    },
    {
        Type = 4,
        Name = "Match Any Enemy Cast (Fallback)",
        Desc = "Fallback mode. Any visible enemy NPC cast or channel will trigger the dimmer, including non-interruptible casts. Turn this on when you want guaranteed behavior even if Midnight blocks interrupt detection.",
        Key = "matchAnyEnemyCasts",
        Value = false,
    },
    {
        Type = 4,
        Name = "Match All Interruptible Casts",
        Desc = "Interrupt mode. Any interruptible enemy NPC cast can trigger the dimmer instead of only your priority spell list. This is best-effort on Midnight because Blizzard can hide interrupt state from addons.",
    ...
    },
    {
        ...
        Desc = "Only enable the dimmer when your character has a supported interrupt ability available in their spellbook. This check is cheap after startup because the result is cached until your talents or spec change.",
    ...
    },
    {
        ...
        Desc = "Only enable the dimmer when your interrupt is currently ready to press. This is slightly more expensive than the known-only check because the mod has to watch cooldown readiness with a short cache.",
    ...
    },
    {
        Type = 4,
        Name = "Debug Output",
        Desc = "Prints decision details to chat so you can see exactly why the dimmer did or did not trigger for a cast. Leave this off outside troubleshooting.",
        Key = "debugEnabled",
        Value = false,
    },
    {
        Type = 4,
        Name = "Debug Successes",
        Desc = "Adds successful trigger messages on top of the normal debug output. This is intentionally noisy and is only useful when diagnosing activation behavior.",
        Key = "debugSuccesses",
        Value = false,
    },
}
...
local export_table = {
    ...
    ["3"] = "Dims other enemy NPC nameplates while a tracked enemy cast is active.",
    ["4"] = "MedaLink",
    ["5"] = 1770000000,
    ["6"] = 2,
    ["7"] = 1,
